      ...
      if (events.length) {
        ...
        events.forEach(ev => {
          ...
          if (splitSummary[0] === "Flight") {
            let flightRef = ev.summary.split(/[()]+/).filter(function(e) {
              return e;
            })[1];
            ...
            let departure = new Date(ev.start.dateTime).toLocaleTimeString();
            let arrival = new Date(ev.end.dateTime).toLocaleTimeString();
        ...
        if (newEvs.length > 0) events = newEvs;
        //end customizing
        ...
        // get data from db, add events to the database and then render the / page with the data as all other users
        GlobalData.findOneAndUpdate({ email }, { events }, (err, obj) => {
          pugRes.redirect("/friends");
        });

        //email: events[0].creator.email
